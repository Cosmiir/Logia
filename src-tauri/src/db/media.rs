...
pub fn get_by_id(conn: &Connection, media_id: i64) -> Result<Option<MediaDetail>> {
    ...
    if let Some(media) = media {
        ...
        let exp_entries = experience_entries::get_by_media_id(conn, media_id)?;
        Ok(Some(MediaDetail { media, images, attachments, genres: media_genres, credits: media_credits, experience_entries: exp_entries }))
    } else {
        Ok(None)
    }
}
...
pub fn insert(conn: &Connection, dto: CreateMediaDto) -> Result<i64> {
    let experience_entries = dto.experience_entries.clone();
    conn.execute(
    ...
    if let Some(entries) = experience_entries {
        experience_entries::replace_for_media(conn, media_id, &entries)?;
    }
    Ok(media_id)
}
...
pub fn update(conn: &Connection, dto: UpdateMediaDto) -> Result<()> {
    ...
    if updates.is_empty() {
        // Even if no media fields changed, we might need to update experience_entries
        if let Some(entries) = &dto.experience_entries {
            experience_entries::replace_for_media(conn, dto.media_id, entries)?;
        }
        return Ok(());
    }
