...
const MediaCreate: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const parsedEntries: ExperienceEntryForm[] = (m.experience_entries || []).map((e) => ({
      date: e.date || '',
      version: e.version || '',
      language: e.language || '',
    }));
    const experienceEntries: ExperienceEntryForm[] = parsedEntries.length > 0 ? parsedEntries : [{ date: m.experience_date || '', version: '', language: '' }];
  ...
  const getChangedFieldGroups = useCallback((): { section: string; color: string; fields: string[] }[] => {
    ...
    try {
      ...
      if (diff('experienceEntries')) progressFields.push(i18next.t('media.experienceDate'));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const experienceEntriesInput = form.experienceEntries.map((e, idx) => ({
        date: e.date || null,
        version: e.version.trim() || null,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6 items-start">
          ...
          <div className="space-y-6">
            ...
            <div className="glass-card rounded-2xl p-6 relative z-20">
              ...
              {/* Experience entries (date + version + language per entry) */}
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setForm((prev) => ({
                        ...prev,
                        experienceEntries: [...prev.experienceEntries, { date: '', version: '', language: '' }],
                        replayCount: prev.experienceEntries.length,
                      }));
                    }}
                    className="ml-auto flex items-center gap-1 text-[10px] text-primary/60 hover:text-primary transition-colors cursor-pointer"
                  >
                    <Plus className="w-3 h-3" />
                    <span>{i18next.t('mediaCreate.addExperience')}</span>
                  </button>
                </div>
                <div className="space-y-2 pl-6">
                  {form.experienceEntries.map((entry, idx) => (
                    <div key={idx} className="flex flex-col gap-1.5 p-2 rounded-lg bg-white/[0.02] border border-white/5">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1">
                          <CustomDatePicker
                            ...
                            onChange={(val) => {
                              setForm((prev) => {
                                const entries = [...prev.experienceEntries];
                                entries[idx] = { ...entries[idx], date: val };
                                return { ...prev, experienceEntries: entries };
                              });
                            }}
                            placeholder="--"
                          />
                        ...
                        {form.experienceEntries.length > 1 && (
                          <button
                            ...
                            onClick={() => {
                              setForm((prev) => ({
                                ...prev,
                                experienceEntries: prev.experienceEntries.filter((_, i) => i !== idx),
                                replayCount: Math.max(0, prev.experienceEntries.length - 2),
                              }));
                            }}
                            className="w-6 h-6 flex items-center justify-center rounded-lg bg-white/5 border border-white/10 text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-all cursor-pointer shrink-0"
                          >
                            <X className="w-3 h-3" />
                          </button>
                        )}
                        ...
                        <div className="flex-1">
                          <input
                            ...
                            onChange={(e) => {
                              setForm((prev) => {
                                const entries = [...prev.experienceEntries];
                                entries[idx] = { ...entries[idx], version: e.target.value };
                                return { ...prev, experienceEntries: entries };
                              });
                            }}
                            placeholder={i18next.t('mediaCreate.versionPlaceholder')}
                            className="w-full px-2 py-1 bg-white/5 border border-white/10 rounded-lg text-xs text-white placeholder-white/20 focus:outline-none focus:border-primary/30"
                          />
                        </div>
                        <LanguagePicker
                          ...
                          onChange={(val) => {
                            setForm((prev) => {
                              const entries = [...prev.experienceEntries];
                              entries[idx] = { ...entries[idx], language: val };
                              return { ...prev, experienceEntries: entries };
                            });
                          }}
                          compact
